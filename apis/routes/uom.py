...
def get_all_uom(db: Session = Depends(get_db),current_user: User = Depends(get_current_user)):
    return uom.readAll(db)

@router.get('/{id}',status_code=status.HTTP_200_OK,response_model=showUom)
def get_uom(id: int, db: Session = Depends(get_db),current_user: User = Depends(get_current_user)):
    return uom.read(id,db)

@router.post("/", status_code=status.HTTP_200_OK, response_model=Message)
def create(request: Uom, db: Session = Depends(get_db),current_user: User = Depends(get_current_user)):
...
def update(id: int, request: Uom,db: Session = Depends(get_db),current_user: User = Depends(get_current_user)):
