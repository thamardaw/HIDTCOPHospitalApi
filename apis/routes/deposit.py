...
@router.get('/cancelled',status_code=status.HTTP_200_OK, response_model=List[DepositSmallDTO])
def get_all_cancelled_deposits(repo=Depends(BillRepository)):
    return BillService(repo).getAllCancelledDeposit()

@router.get('/active/{id}',status_code=status.HTTP_200_OK, response_model=List[DepositSmallDTO])
